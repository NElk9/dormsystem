...
function func_login() {
    ...
    if (User != null && Pass != null && choice != null) {
        axios.get('http://10.30.68.224:8080/login', {
            params: {
                uno: User,
                upass: codePass,
                utype: choice
...
function getRepairInfo() {
    ...
    axios.get('http://10.30.68.224:8080/getRepairstoStu', {
        params: {
            sno: User
        }
    })
...
function getStuInfo() {
    var User = sessionStorage.getItem("CounselorUserName");
    // alert(User);
    axios.get('http://10.30.68.224:8080/StudentUserInfo', {
        params: {
            sno: User
        }
    })
...
function getDormChangeInfo() {
    ...
    axios.get('http://10.30.68.224:8080/getStuChangeDorm', {
        params: {
            sno: User
        }
    })
...
function getDormCheckoutInfo() {
    ...
    axios.get('http://10.30.68.224:8080/getStuCheckout', {
        params: {
            sno: User
        }
    })
...
function personinfo_submit() {
    ...
    axios.post('http://10.30.68.224:8080/StudentUpdateInfo', {
        sno: sno,
        stel: num,
        sclass: cla
    })
...
function accom_Leavesubmit() {
    ...
    if (checkoutdate !== "") {
        ...
        axios.post('http://10.30.68.224:8080/postStuCheckout', {
            sno: sno,
...
function repair_submit() {
    ...
    if (repairdate !== "" && reason !== "" && itemname !== "") {
        ...
        axios.post('http://10.30.68.224:8080/postRepairbySno', {
            sno: sno,
...
function accom_Changesubmit() {
    ...
    if (changedate !== "") {
        ...
        axios.post('http://10.30.68.224:8080/postStuChangeDorm', {
            sno: sno,
...
function leave_submit() {
    ...
    if (leavedate !== "" && returndate !== "" && reason !== "") {
        ...
        axios.post('http://10.30.68.224:8080/postStuLeave', {
            sno: User,
            leavedate: leavedate,
            backdate: returndate,
            reason: reason,
        ...
        if (pass === passCon) {
            ...
            if (pass === passOldUser) {
                ...
                if (pass.match(/(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[^a-zA-Z0-9]).{8,20}/)) {
                    ...
                    axios.post('http://10.30.68.224:8080/updatePassword', {
                        uno: sno,
                        oldpass: codePassOld,
                        newpass: codePass
                    })
